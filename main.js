...
function generate(content, initialCategory = null, targetRowId = null) {
  ...
  title.innerHTML = "現在學習的是" + fullLvlName; // 設定初始標題
  ...
  const dialectInfo = {
      腔, 級, 例外音檔, fullLvlName, generalMediaYr,
...
function buildTableAndSetupPlayback(category, vocabularyArray, dialectInfo, autoPlayTargetRowId = null) {
  ...
  console.log(`Building table for category: ${category}, autoPlayRow: ${autoPlayTargetRowId}`); // 增加日誌
  ...
  for (const line of vocabularyArray) {
    if (line.分類 && line.分類.includes(category) == true) {
      ...
      if (dialectInfo.級 === "初") {no[0] = "0"+no[0];} // 初級特殊處理
      ...
      const 詞目錄 = dialectInfo.目錄級+"/"+dialectInfo.檔腔+"/"+dialectInfo.檔級+dialectInfo.檔腔;
      const 句目錄 = 句目錄級+"/"+dialectInfo.檔腔+"/"+pre112Insertion+dialectInfo.檔級+dialectInfo.檔腔;
  ...
  function playAudio(index) {
      ...
      currentAudio.play().then(() => {
          ...
          console.error(`播放音訊失敗 (索引 ${index}, src: ${currentAudio.src}):`, error);
          // 播放失敗，自動跳到下一個
          currentAudioIndex++;
          playAudio(currentAudioIndex);
      });
  ...
  bookmarkButtons.forEach(button => {
      button.addEventListener('click', function() {
          ...
          const newBookmark = {
            rowId: rowId, percentage: percentageFixed, cat: currentCategoryForBookmark,
            tableName: currentTableNameForBookmark, timestamp: Date.now()
          };
  ...
  audioElements.forEach(audio => {
    ...
    if (audio.dataset.skip !== 'true') {
        audio.addEventListener('play', function() {
            const rowButton = this.closest('tr')?.querySelector('button[data-row-id]');
            if (!rowButton) return;
  ...
  if (!audioControlsDiv) {
      ...
      if (!pauseResumeButton || !stopButton /* || !playAllButton */) {
          console.warn("#audioControls span exists, but buttons missing. Recreating buttons.");
  ...
  // 確保按鈕變數在此處是有效的
    if (pauseResumeButton) {
        pauseResumeButton.onclick = function() { // 使用 onclick 覆蓋舊監聽器
            if (isPlaying) {
                if (isPaused) {
                    currentAudio?.play().catch(e => console.error("恢復播放失敗:", e));
                    isPaused = false;
                    this.innerHTML = '<i class="fas fa-pause"></i>';
                    this.classList.add("ongoing"); this.classList.remove("ended");
                } else {
                    currentAudio?.pause();
                    isPaused = true;
                    this.innerHTML = '<i class="fas fa-play"></i>';
                    this.classList.remove("ongoing"); this.classList.add("ended"); // Or a specific paused style
                }
            }
        };
    } else { console.error("pauseResumeButton not found for binding"); }

    if (stopButton) {
        stopButton.onclick = function() { // 使用 onclick 覆蓋舊監聽器
            if (isPlaying) {
                if (currentAudio) {
                    currentAudio.pause();
                ...
                currentAudio = null;
                if(pauseResumeButton) pauseResumeButton.innerHTML = '<i class="fas fa-pause"></i>';
                if(pauseResumeButton) pauseResumeButton.classList.remove("ongoing"); pauseResumeButton.classList.add("ended");
                this.classList.remove("ongoing"); this.classList.add("ended");
                document.querySelectorAll('.playFromThisRow').forEach(element => {
                   element.classList.remove('ongoing'); element.classList.add("playable");
                });
                removeNowPlaying();
            }
        };
    } else { console.error("stopButton not found for binding"); }
  ...
  playFromRowButtons.forEach(button => {
    button.onclick = function() { // 使用 onclick 覆蓋舊監聽器
      if (isPlaying) { // 如果正在播放，先停止
  ...
  function startPlayingFromRow(buttonElement) {
      ...
      // 更新按鈕狀態
      if(pauseResumeButton) pauseResumeButton.innerHTML = '<i class="fas fa-pause"></i>';
      if(pauseResumeButton) { pauseResumeButton.classList.remove("ended"); pauseResumeButton.classList.add("ongoing"); }
      if(stopButton) { stopButton.classList.remove("ended"); stopButton.classList.add("ongoing"); }
      playFromRowButtons.forEach(element => { element.classList.add('ongoing'); }); // 所有播放按鈕變色
  }
      ...
      if (selectedIndex !== null && !isNaN(parseInt(selectedIndex))) {
        ...
        if (selectedBookmark) {
          ...
          if (dataVarName && typeof window[dataVarName] !== 'undefined') {
            ...
            console.log(`Calling generate for ${dataVarName}, category: ${targetCategory}, row: ${targetRowIdToGo}`); // 增加日誌
...
function mapTableNameToDataVar(tableName) {
  const mapping = {
    "四縣基礎級": "四基", "四縣初級": "四初", "四縣中級": "四中", "四縣中高級": "四中高",
    "海陸基礎級": "海基", "海陸初級": "海初", "海陸中級": "海中", "海陸中高級": "海中高",
    "大埔基礎級": "大基", "大埔初級": "大初", "大埔中級": "大中", "大埔中高級": "大中高",
    "饒平基礎級": "平基", "饒平初級": "平初", "饒平中級": "平中", "饒平中高級": "平中高",
    "詔安基礎級": "安基", "詔安初級": "安初", "詔安中級": "安中", "詔安中高級": "安中高",
